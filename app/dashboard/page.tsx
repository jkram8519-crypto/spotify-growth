...
{showToolsModal && (
  <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.85)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
    <div style={{background:'#0d0020',padding:'30px',borderRadius:'24px',maxWidth:'500px',width:'100%',border:'1px solid #9B59B6',maxHeight:'80vh',overflowY:'auto'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
        <h2 style={{margin:0,fontSize:'22px',fontWeight:'bold'}}>🚀 Les 11 outils IA</h2>
        <button onClick={() => setShowToolsModal(false)}
          style={{background:'none',border:'none',color:'#aaa',cursor:'pointer',fontSize:'20px'}}>✕</button>
      </div>
      {[
        {emoji:'🚀',title:'Pitch Generator IA',desc:'Génère des pitches professionnels pour les curateurs en 10 secondes.',section:'pitch'},
        {emoji:'🗓️',title:'Manager IA',desc:'Planifie automatiquement ta sortie sur 44 jours — de J-30 à J+14.',section:'manager'},
        {emoji:'🎯',title:'Playlist Finder',desc:'Trouve les playlists parfaites avec un score de compatibilité.',section:'playlists'},
        {emoji:'📊',title:'Analytics IA',desc:'Analyse ton taux de save, skip rate et te donne des actions concrètes.',section:'analytics'},
        {emoji:'🎯',title:'Growth Score',desc:'Calcule ton score de croissance Spotify sur 100 points.',section:'growth'},
        {emoji:'🔥',title:'Viral Potentiel',desc:'Analyse ton track et détecte son potentiel viral sur TikTok.',section:'viral'},
        {emoji:'🎨',title:'Profil Artiste',desc:'Analyse ton profil Spotify et optimise chaque élément.',section:'profil'},
        {emoji:'📱',title:'Contenu Social',desc:'Génère du contenu optimisé pour Instagram, TikTok, Twitter.',section:'contenu'},
        {emoji:'🤖',title:'IA Assistant',desc:'Pose tes questions à l\'IA manager et obtiens des conseils pro.',section:'ia'},
        {emoji:'📊',title:'Multi-Plateformes',desc:'Centralise tes stats Spotify, TikTok, Instagram et YouTube.',section:'multi'},
        {emoji:'💬',title:'Feedback',desc:'Donne ton avis et aide à améliorer l\'app.',section:'feedback'},
      ].map((tool, i) => (
        <div key={i}
          onClick={() => { setActiveSection(tool.section); setShowToolsModal(false); }}
...
<div style={{background:'linear-gradient(135deg,#1a0030,#0d0020)',padding:'20px 25px',borderRadius:'15px',marginBottom:'25px',border:'1px solid #2d1040',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
  ...
  <button onClick={() => setShowToolsModal(true)}
    style={{background:'#0d0020',padding:'10px 15px',borderRadius:'10px',textAlign:'center',border:'1px solid #2d1040',cursor:'pointer'}}>
