...
<div className="bg-zinc-900 p-8 rounded-3xl mt-8 col-span-2">
  ...
  <button className="bg-purple-600 text-white px-6 py-3 rounded-xl font-bold w-full mb-6"
    onClick={() => {
      ...
      const color = score >= 70 ? '#1DB954' : score >= 40 ? '#f39c12' : '#e74c3c';
      const label = score >= 70 ? 'Excellent' : score >= 40 ? 'En progression' : 'A ameliorer';
      const tip = score >= 70 ? 'Continue comme ca, tu es sur la bonne voie!' : 'Soumets ton track aux curateurs et ameliore ta presence sur les reseaux.';
