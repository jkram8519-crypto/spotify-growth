...
export async function POST(req: NextRequest) {
  try {
    const { plan, billing, currentPlan } = await req.json();

    // Le code promo n'est proposé qu'aux utilisateurs encore en Free (jamais payé)
    const allowPromo = (currentPlan || 'Free') === 'Free';
